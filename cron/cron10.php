    ...
    if($CMSNT->site('status_connect_api') == 1){
        ...
        foreach($CMSNT->get_list(" SELECT * FROM `connect_api` WHERE `status` = 1 AND `type` = 'API_10' ") as $website){
            ...
            foreach($data['Data'] as $account){
                ...
                if(!$rowProduct = $CMSNT->get_row(" SELECT * FROM `products` WHERE `id_api` = '".check_string($account['MailCode'])."' AND `id_connect_api` = '".$website['id']."' ")){
                    // LẤY ID CATEGORY
                    $id_api = 0;
                    $isInsert = $CMSNT->insert('products', [
                        'user_id'           => $website['user_id'],
                        'category_id'       => $id_api,
                        'id_api'            => check_string($account['MailCode']),
