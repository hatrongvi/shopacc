...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $isBuy = $User->RemoveCredits($getUser['id'], $total_payment, "".__('Thanh toán đơn hàng mua tài khoản')." #".$trans_id);
    if ($isBuy) {
        ...
        if($row['id_api'] != 0){
            ...
            if($row_connect_api['type'] == 'API_10'){
                $response = curl_get($row_connect_api['domain'].'mail/buy?apikey='.$row_connect_api['password'].'&mailcode='.$row['id_api'].'&quantity='.$amount);
                $data = json_decode($response, true);
                if($data['Code'] != 0){
                    $User->RefundCredits($getUser['id'], $total_payment, "[Error] Hoàn tiền đơn hàng mua tài khoản #".$trans_id." - ".$row['name']);
